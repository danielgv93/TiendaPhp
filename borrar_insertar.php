...
if (isset($_POST["insertarMovil"]) && $_POST["modelo"] && $_POST["precio"]) {
    try {
        if (($targetFile = guardarImagenDispositivo($_POST["modelo"], $_FILES["imagen"])) !== false) {
            if (Database::getInstance()->addMovil(new Movil(0, $_POST["modelo"], $_POST["precio"], $_POST["gama"],
                $_POST["anio"], $_POST["ram"], $_POST["almacenamiento"], $_POST["procesador"], $_POST["bateria"],
                $_POST["pulgadas"], 0, $targetFile, 0, $_POST["camara"], $_POST["notch"])))
...
if (isset($_POST["insertarReloj"]) && $_POST["modelo"] && $_POST["precio"]) {
    try {
        if (($targetFile = guardarImagenDispositivo($_POST["modelo"], $_FILES["imagen"])) !== false) {
            if (Database::getInstance()->addReloj(new Reloj(0, $_POST["modelo"], $_POST["precio"], $_POST["gama"], $_POST["anio"],
                $_POST["ram"], $_POST["almacenamiento"], $_POST["procesador"], $_POST["bateria"], $_POST["pulgadas"], 0,
                $targetFile, 0, $_POST["sim"])))
...
if (isset($_POST["borrar"])) {
    try {
        if (Database::getInstance()->borrarModelo($_POST["modelo"])) {
    ...
    <div class="container">
        <div class="row justify-content-center mt-5">
            <div class="col-6">
                ...
                <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="post" class="p-2 mb-3"
                      name="formulario">
                    <legend class="text-center header">Modificar stock</legend>
                    <div class="form-group">
                        <select name="modelo" class="form-control">
                            <?php
                            $dispositivos = Database::getInstance()->getDispositivos();
                            foreach ($dispositivos as $id => $dispositivo) : ?>
                                <option value="<?= $id ?>" <?php if (isset($_POST["elegirModelo"]) && ($id == $_POST["modelo"])) echo "selected" ?>><?= $dispositivo->getModelo() ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <?php if (isset($_POST["elegirModelo"])) : ?>
                    <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="post" class="p-2 mb-3"
                          name="formulario">
                        <legend class="text-center header">Modelo
                            elegido<?= " " . $dispositivos[$_POST["modelo"]]->getModelo() ?></legend>
                        <div class="form-group">
                            <input type="hidden" class="form-control" name="idModelo" value="<?= $_POST["modelo"] ?>">
                            <label for="numero">Número de stock:</label>
                            <input type="number" class="form-control" name="stockDispositivo"
                                   value="<?= $dispositivos[$_POST["modelo"]]->getModelo() ?>">
                        </div>
                ...
                <?php if (isset($_POST["submitTipoDisp"]) && $_POST["tipoDispositivo"] == "movil") : ?>
                    <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="post"
                          enctype="multipart/form-data" class=" -primary p-2 mb-3 rounded">
                ...
                <?php endif; ?>
                <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="post"
                      class=" -primary p-2 mt-3 rounded">
